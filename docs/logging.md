...
This guide defines logging conventions for Symphony Pi so runtime failures can be diagnosed quickly.
...
When logging runtime execution lifecycle events, include:

- `session_id`: the runtime session or turn identifier used by the orchestrator.
...
- `Pi.RpcBackend`: log session start/completion/error with issue context and `session_id`.

## Checklist For New Logs

- Is this event tied to a Linear issue? Include `issue_id` and `issue_identifier`.
- Is this event tied to a runtime session? Include `session_id`.
