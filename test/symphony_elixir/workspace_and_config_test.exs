defmodule SymphonyElixir.WorkspaceAndConfigTest do
  ...
  test "config reads defaults for optional settings" do
    ...
    write_workflow_file!(Workflow.workflow_file_path(),
      ...
      pi_read_timeout_ms: nil,
      pi_stall_timeout_ms: nil
    )
    ...
    assert config.pi.session_subdir == ".symphony-pi/session"
    assert config.pi.turn_timeout_ms == 3_600_000
    assert config.pi.read_timeout_ms == 5_000
    assert config.pi.stall_timeout_ms == 300_000
    ...
    write_workflow_file!(Workflow.workflow_file_path(), pi_turn_timeout_ms: "bad")
    assert {:error, {:invalid_workflow_config, message}} = Config.validate!()
    assert message =~ "pi.turn_timeout_ms"

    write_workflow_file!(Workflow.workflow_file_path(), pi_read_timeout_ms: "bad")
    assert {:error, {:invalid_workflow_config, message}} = Config.validate!()
    assert message =~ "pi.read_timeout_ms"

    write_workflow_file!(Workflow.workflow_file_path(), pi_stall_timeout_ms: "bad")
    assert {:error, {:invalid_workflow_config, message}} = Config.validate!()
    assert message =~ "pi.stall_timeout_ms"
  end

  test "config resolves $VAR references for env-backed secret and path values" do
    workspace_env_var = "SYMP_WORKSPACE_ROOT_#{System.unique_integer([:positive])}"
    api_key_env_var = "SYMP_LINEAR_API_KEY_#{System.unique_integer([:positive])}"
    pi_bin_env_var = "SYMP_PI_BIN_#{System.unique_integer([:positive])}"
    ...
    previous_pi_bin = System.get_env(pi_bin_env_var)

    System.put_env(workspace_env_var, workspace_root)
    System.put_env(api_key_env_var, api_key)
    System.put_env(pi_bin_env_var, pi_bin)

    on_exit(fn ->
      restore_env(workspace_env_var, previous_workspace_root)
      restore_env(api_key_env_var, previous_api_key)
      restore_env(pi_bin_env_var, previous_pi_bin)
    end)

    write_workflow_file!(Workflow.workflow_file_path(),
      tracker_api_token: "$#{api_key_env_var}",
      workspace_root: "$#{workspace_env_var}",
      pi_command: "$#{pi_bin_env_var}"
    )

    config = Config.settings!()
    assert config.tracker.api_key == api_key
    assert config.workspace.root == Path.expand(workspace_root)
    assert config.pi.command == pi_bin
  end
  ...
  test "schema parse normalizes env-backed fallbacks" do
    ...
    pi_bin_env = "SYMP_PI_BIN_#{System.unique_integer([:positive])}"
    ...
    previous_linear_api_key = System.get_env("LINEAR_API_KEY")
    previous_pi_bin = System.get_env(pi_bin_env)
    ...
    System.put_env(pi_bin_env, "/opt/pi")
    ...
    on_exit(fn ->
      ...
      restore_env(pi_bin_env, previous_pi_bin)
    end)
  ...
  test "workflow prompt is used when building base prompt" do
    workflow_prompt = "Workflow prompt body used as Pi instruction."

    write_workflow_file!(Workflow.workflow_file_path(), prompt: workflow_prompt)
    assert Config.workflow_prompt() == workflow_prompt
  end

  test "worker config defaults to empty ssh_hosts" do
    {:ok, settings} = Schema.parse(%{})
    assert settings.worker.ssh_hosts == []
    assert is_nil(settings.worker.max_concurrent_agents_per_host)
  end

  test "worker config parses ssh_hosts and max_concurrent_agents_per_host" do
    {:ok, settings} =
      Schema.parse(%{
        "worker" => %{
          "ssh_hosts" => ["worker1.example.com", "worker2.example.com:2222"],
          "max_concurrent_agents_per_host" => 3
        }
      })

    assert settings.worker.ssh_hosts == ["worker1.example.com", "worker2.example.com:2222"]
    assert settings.worker.max_concurrent_agents_per_host == 3
  end

  test "worker config rejects non-positive max_concurrent_agents_per_host" do
  ...
  test "pi config accepts extension_dir" do
    {:ok, settings} =
      Schema.parse(%{
        "pi" => %{
          "extension_dir" => "/opt/symphony/extensions"
        }
      })

    assert settings.pi.extension_dir == "/opt/symphony/extensions"
  end

  test "pi config extension_dir defaults to nil" do
    {:ok, settings} = Schema.parse(%{})
    assert is_nil(settings.pi.extension_dir)
  end
