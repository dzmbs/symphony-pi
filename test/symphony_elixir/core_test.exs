defmodule SymphonyElixir.CoreTest do
  ...
  defmodule SessionTrackingBackend do
    ...
    def run_turn(session, _prompt, _issue, _opts) do
      next_session = %{session | turn_number: session.turn_number + 1}
      {:ok, %{result: :turn_completed, session_id: "test-turn-#{next_session.turn_number}"}, next_session}
    end

    @impl true
    def stop_session(%{stopped_recipient: recipient} = session) do
      send(recipient, {:backend_stopped, session})
      :ok
    end
  ...
  test "config defaults and validation checks" do
    ...
    assert Config.settings!().pi.command == "   "

    write_workflow_file!(Workflow.workflow_file_path(), pi_command: "/bin/sh")
    assert :ok = Config.validate!()
  ...
  test "current WORKFLOW.md file is valid and complete" do
    ...
    assert is_map(hooks)
    assert Map.get(hooks, "after_create") =~ "git clone --depth 1 https://github.com/dzmbs/symphony-pi ."
  ...
  test "in-repo WORKFLOW.md renders correctly" do
    ...
    assert prompt =~ "follow the `land` skill"
  ...
  test "agent runner stops the final updated backend session" do
    Application.put_env(:symphony_elixir, :backend_module_override, SessionTrackingBackend)
    ...
    issue = %Issue{
      id: "issue-stop-final-session",
      identifier: "MT-999",
      title: "Stop final session",
      description: "Ensure cleanup sees the last updated backend session",
      state: "In Progress",
      url: "https://example.org/issues/MT-999",
      labels: []
    }
    ...
    state_fetcher = fn [_issue_id] ->
      attempt = Process.get(:agent_stop_fetch_count, 0) + 1
      Process.put(:agent_stop_fetch_count, attempt)
