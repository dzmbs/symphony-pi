defmodule Mix.Tasks.Workspace.BeforeRemoveTest do
  ...
  @default_repo "dzmbs/symphony-pi"
  ...
  test "uses current branch for lookup when branch option is omitted" do
    with_fake_gh_and_git(
      ...
      fn log_path ->
        ...
        assert log =~
                 "pr list --repo #{@default_repo} --head feature/workpad --state open --json number --jq .[].number"

        assert log =~ "pr close 101 --repo #{@default_repo}"
        assert log =~ "pr close 102 --repo #{@default_repo}"
      end
    )
  end
  ...
  test "formats close failures without command stderr output" do
    with_fake_gh(
      ...
      fn log_path ->
        ...
        assert log =~
                 "pr list --repo #{@default_repo} --head feature/no-output --state open --json number --jq .[].number"
  ...
  test "no-ops when PR list fails for current branch" do
    with_fake_gh(
      ...
      fn log_path ->
        ...
        assert log =~
                 "pr list --repo #{@default_repo} --head feature/list-fails --state open --json number --jq .[].number"
