defmodule SymphonyElixir.Orchestrator do
  @moduledoc """
  Polls Linear and dispatches repository copies to agent-backend workers.
  """
  ...
  alias SymphonyElixir.{AgentRunner, AutoReview, Config, StatusDashboard, Tracker, Workspace}
  ...
  def handle_info(
    ...
    case Map.get(running, issue_id) do
      ...
      running_entry ->
        {updated_running_entry, token_delta} = integrate_runtime_update(running_entry, update)
  ...
  defp reconcile_issue_state(%Issue{} = issue, state, active_states, terminal_states) do
    cond do
      ...
      protected_auto_review_handoff_state?(issue) ->
  ...
  defp protected_auto_review_handoff_state?(%Issue{state: state_name}) when is_binary(state_name) do
    AutoReview.enabled?() and
      normalize_issue_state(state_name) == normalize_issue_state(AutoReview.agent_review_state())
  end

  defp protected_auto_review_handoff_state?(_issue), do: false
  ...
  defp reconcile_stalled_running_issues(%State{} = state) do
    timeout_ms = runtime_stall_timeout_ms()
  ...
  defp restart_stalled_issue(state, issue_id, running_entry, now, timeout_ms) do
    ...
    if is_integer(elapsed_ms) and elapsed_ms > timeout_ms do
      ...
      |> schedule_issue_retry(issue_id, next_attempt, %{
        identifier: identifier,
        error: "stalled for #{elapsed_ms}ms without runtime activity"
      })
    else
      state
    end
  ...
  defp last_activity_timestamp(running_entry) when is_map(running_entry) do
    Map.get(running_entry, :last_runtime_timestamp) || Map.get(running_entry, :started_at)
  end
  ...
  defp schedule_issue_retry(%State{} = state, issue_id, attempt, metadata)
    ...
    worker_host = metadata[:worker_host] || Map.get(previous_retry, :worker_host)
  ...
  defp handle_active_retry(state, issue, attempt, metadata) do
    ...
    if retry_candidate_issue?(issue, terminal_state_set()) and
         ...
         worker_slots_available?(state) do
  ...
  defp pick_retry_worker_host(metadata) when is_map(metadata) do
    Map.get(metadata, :worker_host)
  end

  defp pick_retry_worker_host(_metadata), do: nil

  defp maybe_put_runtime_value(entry, _key, nil), do: entry
  defp maybe_put_runtime_value(entry, key, value), do: Map.put(entry, key, value)
  ...
  defp integrate_runtime_update(running_entry, %{event: event, timestamp: timestamp} = update) do
  ...
  defp runtime_pid_for_update(_existing, %{runtime_pid: pid})
       when is_binary(pid),
       do: pid
  ...
  defp runtime_pid_for_update(_existing, %{runtime_pid: pid}) when is_list(pid),
    do: to_string(pid)

  defp runtime_pid_for_update(existing, _update), do: existing
  ...
  defp summarize_runtime_update(update) do
    %{
      event: update[:event],
      message: update[:raw] || update[:payload],
  ...
  defp runtime_stall_timeout_ms do
    Config.settings!().pi.stall_timeout_ms
  end
